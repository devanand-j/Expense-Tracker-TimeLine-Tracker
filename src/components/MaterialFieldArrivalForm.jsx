...
export default function MaterialFieldArrivalForm({ materials, onSubmit, isLoading }) {
  ...
  const validateForm = () => {
    if (!formData.personName || !formData.personName.trim()) {
      toast.error('Please enter the name of the person confirming arrival');
      return false;
    }
    const allChecked = Object.values(formData.checklist).every((v) => v === true);
    if (!allChecked) {
      toast.error('Please verify all materials at field location');
      return false;
    }
    if (!formData.materialSetPhotoUrl) {
      toast.error('Photo of materials at field location is required');
      return false;
    }
    return true;
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800">
        <h2 className="mb-4 text-2xl font-bold text-slate-900 dark:text-slate-100">
          Material Arrival at Field Location
        </h2>
        <p className="text-slate-600 dark:text-slate-400">
          Confirm safe arrival of all materials at the field location
        </p>
      </div>

      <div className="rounded-lg border-l-4 border-blue-500 bg-blue-50 p-4 dark:border-blue-400 dark:bg-blue-900/20">
        <p className="text-sm text-blue-800 dark:text-blue-300">
          Verify that all materials from the warehouse pickup have arrived in good condition
        </p>
      </div>
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800">
        <label className="block">
          <p className="text-sm font-medium text-slate-700 dark:text-slate-300">
            👤 Name of Person Confirming Arrival <span className="text-red-500">*</span>
          </p>
          <input
            type="text"
            value={formData.personName}
            onChange={handlePersonNameChange}
            placeholder="Enter full name of person at field location"
            maxLength="100"
          ...
          <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
            Required: This records who confirmed arrival and at what time
          </p>
        </label>
      </div>
      ...
      <MaterialPhotoUpload
        label="Photo of Materials at Field Location"
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800">
        <label className="block">
          ...
          <textarea
            ...
            placeholder="Add any observations about the materials at field location..."
            rows="4"
            className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-slate-600 dark:bg-slate-700 dark:text-white"
          />
        </label>
      ...
      >
        {isLoading ? 'Submitting...' : 'Confirm Field Arrival'}
      </button>
    </form>
  );
}
